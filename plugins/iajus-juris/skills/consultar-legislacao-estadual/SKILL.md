...
description: Consulta legislação ESTADUAL e MUNICIPAL brasileira ao vivo (estados com texto integral SP/MG/BA, DF resolve+ementa, RJ best-effort; dezenas de municípios cobertos) pelo MCP iaJus, na fonte oficial de cada UF/município (assembleia, câmara, diário oficial). Acione sempre que o usuário pedir uma lei/decreto/norma de um ESTADO ou MUNICÍPIO específico, o texto íntegra, "lei estadual nº Y de SP", "decreto do RJ número Z", "lei municipal de Porto Alegre", "o que a UF X cobre". A consulta é por UF (+ município) + tipo + número + ano (a fonte oficial é a verdade — nunca cite de memória). NÃO use para legislação FEDERAL (skill consultar-legislacao) nem para precedentes/acórdãos/súmulas (skill pesquisar-jurisprudencia).
